...
import FigureResult from './FigureResult';
...
export default class FigureResults extends React.Component {
  ...
  render() {
    console.log("searchREsults FIGURE=", this.state.results);
  	return(
  	 <div>
	    <div className="Header">
	    	<h4>Figure Results <span className="NumResults">({this.state.results.length})</span>
        	</h4>
    	</div> 
		  ...
		  {(array) => (
		     array.map((item, i) => (
		        <FigureResult item={item} index={i} key={i} searchIndex={this.props.searchIndex} /> 
		     )
		  ))}
		</ShowMoreResults>
	</div>
